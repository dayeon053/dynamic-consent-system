...
public class LlmPromptTemplate {
    ...
    public static String buildAnalysisPrompt(String companyName, String policyText) {
        return "당신은 개인정보 처리방침 전문 분석가입니다.\n"
                ...
                + "- riskSummary는 전문 용어 없이 일반 사용자가 읽는 요약 문장이다. \"어떤 정보를\n"
                + "  어떤 목적으로 가져가서 위험한지\"가 드러나야 하며, 위험도가 가장 높은 동의 항목\n"
                + "  1~2개를 근거로 삼아 한 문장으로 작성한다(예: \"위치정보와 행태정보를 맞춤형\n"
                + "  광고 목적으로 수집하고 광고 파트너에게 제공해 위험도가 높습니다.\")\n\n"

                + "=== 출력 형식 ===\n"
                + "{\n"
                + "  \"companyName\": \"" + companyName + "\",\n"
                + "  \"riskSummary\": \"사용자가 읽는 위험 요약 한 문장\",\n"
                + "  \"consentItems\": [\n"
                + "    {\n"
                + "      \"itemName\": \"동의 항목명\",\n"
